...
import {HttpClient, amazonMarketplaces, MWS, Order, OrderStatusEnum} from '@scaleleap/amazon-mws-api-sdk';
...
export const checkForOrders = functions.https.onRequest(async (req, res) => {
    const now = new Date();
    const mws = getMws();
    const data: SaveData = await (await db.collection(DATA_COLLECTION).doc(DATA_DOC).get()).data() as SaveData || {};
    // @ts-ignore
...
function getOrderDiffs(oldOrders: Order[], newOrders: Order[]) {
    const oldOrdersById = new Map(oldOrders.map(order => [order.AmazonOrderId, order]));
    const newOrdersById = new Map(newOrders.map(order => [order.AmazonOrderId, order]));
    ...
    for (const [orderId, newOrder] of newOrdersById.entries()) {
        if (!oldOrdersById.has(orderId)) {
            ...
            if (newOrder.OrderStatus === OrderStatusEnum.Shipped && oldOrder.OrderStatus !== OrderStatusEnum.Shipped) {
